...
enum class FontSizeUnit(val code: String) {

    PX("px"), DP("dp"), SP("sp");

    companion object {
        fun formCode(code: String?): FontSizeUnit {
            if (code == null) return SP
            return values().firstOrNull { it.code == code.lowercase() } ?: SP
        }
    }
}
...
internal class MethodCallHandlerImpl(private var context: Context) : MethodCallHandler {
    ...
    override fun onMethodCall(call: MethodCall, result: MethodChannel.Result) {
        Log.i("onMethodCall", "onMethodCall    result:${result}")
        when (call.method) {
            "showToast" -> {
                ...
                val fontSizeUnit = FontSizeUnit.formCode(call.argument<String>("fontSizeUnit"))
                ...
                if (bgcolor != null) {
                    ...
                    if (fontSize != null) {
                        when (fontSizeUnit) {
                            FontSizeUnit.SP -> text.setTextSize(TypedValue.COMPLEX_UNIT_SP, fontSize.toFloat())
                            FontSizeUnit.PX -> text.setTextSize(TypedValue.COMPLEX_UNIT_PX, fontSize.toFloat())
                            FontSizeUnit.DP -> text.setTextSize(TypedValue.COMPLEX_UNIT_DIP, fontSize.toFloat())
                        }
                        Log.i("onMethodCall", "fontSizeUnit: $fontSizeUnit  fontSize: $fontSize")
                    }
                    if (textcolor != null) {
                    ...
                    if (Build.VERSION.SDK_INT < Build.VERSION_CODES.R) {
                        ...
                        if (fontSize != null) {
                            when (fontSizeUnit) {
                                FontSizeUnit.SP -> textView.setTextSize(TypedValue.COMPLEX_UNIT_SP, fontSize.toFloat())
                                FontSizeUnit.PX -> textView.setTextSize(TypedValue.COMPLEX_UNIT_PX, fontSize.toFloat())
                                FontSizeUnit.DP -> textView.setTextSize(TypedValue.COMPLEX_UNIT_DIP, fontSize.toFloat())
                            }
                            Log.i("onMethodCall", "fontSizeUnit: $fontSizeUnit  fontSize: $fontSize")
                        }
                        if (textcolor != null) {
